...
// src/use-cases/tags/create.ts
var CreateTagUserCase = class {
...
// src/use-cases/tags/create.spec.ts
...
describe("Create Tag Use Case", () => {
  ...
  it("should be able to user create a tag", async () => {
    const user = await usersUseCase.create({
      ...
      password: await (0, import_bcryptjs.hash)("123456", 6)
    });
    ...
    const tags = note.arr_tags.map(async (item) => {
      ...
      globalExpect(tag.id).toEqual(globalExpect.any(String));
      return tag;
    });
    globalExpect(tags).toHaveLength(2);
  });
});
