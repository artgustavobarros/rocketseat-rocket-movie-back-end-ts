...
// src/repositories/in-memory-repositories/in-memory-users-repository.ts
...
(function(module2, exports2) {
  (function() {
    (function(chaiSubset2) {
      ...
      function compare3(expected, actual) {
        if (expected === actual) {
          return true;
        }
...
// src/use-cases/users/update.spec.ts
...
describe("Update User Use Case", () => {
  ...
  it("should be able to change users name", async () => {
    await useCase.create({
      ...
      password: await (0, import_bcryptjs3.hash)("123456", 6)
    });
    const { user } = await sut.execute({
      id: "1",
      name: "Changed Name"
    });
    globalExpect(user.name).toBe("Changed Name");
  });
  it("should be able to change users email", async () => {
    ...
    globalExpect(user.email).toBe("changedemail@email.com");
  });
